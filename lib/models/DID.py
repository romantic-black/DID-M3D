...
class DID(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.vis_depth = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),  # inplace 即 原地操作
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
    ...
    def forward(self, input, coord_ranges, calibs, targets=None, K=50, mode='train'):
        """
        input: [B, 3, 384, 1280]
        coord_ranges: [B, 2, 2] = [[0, 0], [1242, 375]]
        calibs: [B, 3, 4]
        """
    ...
    def get_roi_feat_by_mask(self, feat, box2d_maps, inds, mask, calibs, coord_ranges, cls_ids):
        """
        box2d_maps: [B, 5, 96, 320]
        """
        BATCH_SIZE, _, HEIGHT, WIDE = feat.size()   # 这里变成 RoI 长宽
        ...
        if num_masked_bin != 0:
            ...
            scale_box2d_masked = extract_input_from_tensor(box2d_maps, inds, mask)  # [N, 5]
            # get roi feature
            roi_feature_masked = roi_align(feat, scale_box2d_masked, [7, 7])        # [N, 64, 7, 7]
            # get coord range of each roi
            coord_ranges_mask2d = coord_ranges[scale_box2d_masked[:, 0].long()]     # [N, 2, 2]

            # map box2d coordinate from feature map size domain to original image size domain
            box2d_masked = torch.cat([scale_box2d_masked[:, 0:1],   # 变回降采样前 (注意不是数据增强前)
                                      scale_box2d_masked[:, 1:2] / WIDE * (coord_ranges_mask2d[:, 1, 0:1]
                                                                           - coord_ranges_mask2d[:, 0, 0:1])
                                      + coord_ranges_mask2d[:, 0, 0:1],
                                      scale_box2d_masked[:, 2:3] / HEIGHT * (coord_ranges_mask2d[:, 1, 1:2]
                                                                             - coord_ranges_mask2d[:, 0, 1:2])
                                      + coord_ranges_mask2d[:, 0, 1:2],
                                      scale_box2d_masked[:, 3:4] / WIDE * (coord_ranges_mask2d[:, 1, 0:1]
                                                                           - coord_ranges_mask2d[:, 0, 0:1])
                                      + coord_ranges_mask2d[:, 0, 0:1],
                                      scale_box2d_masked[:, 4:5] / HEIGHT * (coord_ranges_mask2d[:, 1, 1:2]
                                                                             - coord_ranges_mask2d[:, 0, 1:2])
                                      + coord_ranges_mask2d[:, 0, 1:2]], 1)
            ...
            coords_in_camera_coord = torch.cat([self.project2rect(roi_calibs, torch.cat(
                [box2d_masked[:, 1:3],
                 torch.ones([num_masked_bin, 1]).to(device_id)], -1))[:, :2],
                                                self.project2rect(roi_calibs,
                                                                  torch.cat([box2d_masked[:, 3:5],
                                                                                         torch.ones(
                                                                                             [num_masked_bin, 1]).to(
                                                                                             device_id)], -1))[:, :2]],
                                               -1)
            coords_in_camera_coord = torch.cat([box2d_masked[:, 0:1], coords_in_camera_coord], -1)
            # generate coord maps   [N, 2, 7, 7]
            ...
            # compute scale factor  降采样后 / 降采样前
            ...
            vis_depth = self.vis_depth(roi_feature_masked).squeeze(1)   # 把公式去除，直接预测
    ...
    def get_roi_feat(self, feat, inds, mask, ret, calibs, coord_ranges, cls_ids):
        ...
        box2d_centre = coord_map + ret['offset_2d']  # [B, 2, 96, 320]
        box2d_maps = torch.cat([box2d_centre - ret['size_2d'] / 2, box2d_centre + ret['size_2d'] / 2], 1)
        # [B, 5, 96, 320], 元素为 B 索引, 左上角点, 右下角点
